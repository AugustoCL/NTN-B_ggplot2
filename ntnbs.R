...
# load tidyverse and font packages
library(tidyverse)
library(ggthemes)
library(extrafont)
font_import()
...
NTNB %>% 
  ...
  filter(matur.date %in% venc_ntnbs, 
         # filter by date of negociation
         ref.date > as.Date("2019-08-01"), 
         # exclude zero coupons bonds 
  ...
  geom_line(size = 1.5) +
  geom_vline(xintercept = as.Date("2020-03-10"), linetype = "dashed", color = "darkgrey", size = .5) +
  geom_vline(xintercept = as.Date("2020-04-23"), linetype = "dashed", color = "darkgrey", size = .5) +
  geom_label(aes(x = as.Date("2020-03-15"), y = 1, label = "Moro Day"), 
             ...
             family= custom_font, 
             size = 3) + 
  geom_curve(aes(x = as.Date("2020-04-01"), y = 1.2, xend = as.Date("2020-04-21"), yend = 1.7), 
             colour = "#555555", 
             size= 0.6, 
             curvature = -0.3,
  ...
  geom_label(aes(x = as.Date("2020-01-16"), y = 5, label = "Corona Day"), 
             ...
             family= "Gadugi", 
             size = 3) + 
  geom_curve(aes(x = as.Date("2020-02-11"), y = 5.2, xend = as.Date("2020-03-08"), yend = 5.7), 
             colour = "#555555", 
             size= 0.6, 
             curvature = -0.4,
             arrow = arrow(length = unit(0.02, "npc"))) +
